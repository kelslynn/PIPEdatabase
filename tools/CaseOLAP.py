...
class CaseSlim:
	...
	def bm25_df_paper(self, df, max_df, tf, dl, avgdl, k=1.2, b=0.5, multiplier=3):
		score = tf * (k + 1) / (tf + k * (1 - b + b * (dl / avgdl))) # normalized term frequency
		df_factor = math.log(1 + df, 2) / math.log(1 + max_df, 2) # normalized document frequency
		score *= df_factor
		score *= multiplier
		return score

    # Distinctiveness Calculation using Softmax Regression
    ...
    # Ranked list of representative phrases
	...
	def agg_phrase_cnt_df(self, freq_data, selected_docs = None):
		phrase_cnt = {} # total count of phrase in all documents
		phrase_df = {} # number of documents that contain the phrase (it appears at least once)
